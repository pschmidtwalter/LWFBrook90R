...
As mentioned, `run_multi_LWFB90()` is a wrapper for `run_LWFB90()`. Additionally, it takes a data.frame `paramvar`, that contains variable parameter values in columns and their realizations in rows. For each row in `paramvar`, the respective parameter values in `param_b90` are replaced by name, and `run_LWFB90()` is executed. 
