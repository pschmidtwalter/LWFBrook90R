...
After the simulation has finished, a list with model output is returned, along
with the model input (`options_b90`, `param_b90` and derived daily vegetation
properties `standprop_daily`):

```{r}
str(b90res, max.level = 1)
```

The list entry `output` contains date variables ('yr', 'mo', 'da', 'doy'),
daily actual evaporation fluxes ('evap', 'tran', 'irvp', 'isvp', 'slvp',
'snvp'), potential evaporation fluxes ('pint', 'ptran', 'pslvp'), soil water fluxes ('flow', 'vrfln') and state variables ('swat', 'awat', 'relawat'). For a detailed description of all output variables refer to the
...
Another second result set (list item `layer_output`) contains daily soil moisture state
variables and belowground water fluxes of the individual soil layers, organized in
