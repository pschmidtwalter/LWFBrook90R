...
vignette: >
  %\VignetteIndexEntry{intro_LWFB90}
...
The next step is to select output variables and their temporal resolution. Sets of output variables can be selected in a `[7,5]`-matrix, which is used as argument in `runLWFB90()`. To create the matrix, use the function `setoutput_LWFB90()`. You can modify the default output selection by flagging groups of output variables with `0` and `1`. The default selection comprise annual, monthly and daily evapotranspiration ('Evap') datasets, and layer by layer daily soil moisture state variables ('Swat'):
```{r}
...
Now we are ready to perform the single-run simulation using the function `runLWFB90()`:
```{r, eval = F}
...
With 'LWFBrook90R', parallelized multi-run simulations can be performed conveniently, extending the basic single-run applications using the function `runLWFB90()` described in the previous section. Two different multi-run functions exist for two different problems:
...
```{r}
LAI_b90 <-  makeSeasLAI(method = options_b90$lai_method,
                        year = 2003,
...
`makeSeasLAI()` also provides other shape functions, that require additional parameters. For example, the model control option `options_b90$lai_method = 'linear'` uses value pairs of day-of-year and leaf area index as fraction of `maxlai` passed from parameters `param_b90$lai.doy` and `param_b90$lai.frac`. The doy/value-pairs are then used to interpolate the intra-annual course of leaf area index to a daily time series.
...
LAI_linear <-  makeSeasLAI(method = options_b90$lai_method,
                           year = 2003,
                           maxlai = param_b90$maxlai,
...
LAI_coupmodel <-  makeSeasLAI(method = options_b90$lai_method,
                              year = 2003,
                              maxlai = param_b90$maxlai,
                              budburst_doy = param_b90$budburstdoy,
                              leaffall_doy = param_b90$leaffalldoy,
...
lai_variation <- makeSeasLAI(method = options_b90$lai_method,
                              year = years,
                              maxlai = param_b90$maxlai,
                              budburst_doy = param_b90$budburstdoy,
                              leaffall_doy = param_b90$leaffalldoy,
...
text(x = as.Date(paste(param_b90$budburstdoy, years),format = "%j %Y"),
     y = -1.3, paste("doy", param_b90$budburstdoy), xpd = T)
```
...
Like the leaf area index parameters and budburst/leaffall-dates, it is also possible to provide vectors of values for stand height, stem area index, and stand density to generate between-year variation of stand characteristics. From the yearly values, daily values are interpolated using the function `approx_standprop`. The `approx.method`- argument of the function defines how to interpolate the yearly values passed by `y`. Within `runLWFB90`, the option `options_b90$standprop_interp` is passed to the `approx.method`- argument of `approx_standprop`. The default interpolation method 'constant' results in a yearly changing step function, while 'linear' interpolates the values:
```{r}
...
standprop_daily <- runLWFB90(options_b90 = options_b90,
                                       param_b90 = param_b90,
                                       climate = slb1_meteo,
                                       soil = soil,
                                       output = output, 
                                       run = F)$standprop_daily
```
...
roots_beta <- makeRootden(soilnodes = c(max(slb1_soil$upper), slb1_soil$lower), 
                              maxrootdepth = param_b90$maxrootdepth,
                              beta = param_b90$betaroot,
                              method = options_b90$root_method)
```

A second option to define the root distribution for the soil layers is to provide value pairs of soil depth and root density, that are interpolated to midpoints of the soil layers. The value pairs are  passed from the `rootden_table`-entry of the parameter list to `mrunLWFB90()` As an example, we set up a hypothetical root density depth distribution:
...
roots_table <- makeRootden(soilnodes = c(max(slb1_soil$upper), slb1_soil$lower), 
                              method = options_b90$root_method, 
...
roots_linear <- makeRootden(soilnodes = c(max(slb1_soil$upper), slb1_soil$lower), 
                              maxrootdepth = param_b90$maxrootdepth,
                              method = options_b90$root_method)
options_b90$root_method <- 'const'
roots_constant <- makeRootden(soilnodes = c(max(slb1_soil$upper), slb1_soil$lower), 
                              maxrootdepth = param_b90$maxrootdepth,
                              method = options_b90$root_method)
