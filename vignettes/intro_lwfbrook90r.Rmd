...
`runLWFB90` thereby derives the daily stand properties (`lai`, `sai`, `height`, `densef`, `age`) and root distribution from parameters, and passes climate, vegetation properties and parameters to the Fortran dynamic library. After the simulation has finished, `runLWFB90` returns the selected output (if `read.output = TRUE`). The return value is a list containing the model output (as specified by the `output`-argument), along with the model input (`options_b90`, `param_b90` and derived daily vegetation properties `standprop_daily`), if desired (`rtrn_input = TRUE`). The model output items in the result list are data.tables. In this example, the EVAPDAY.ASC data.table contains daily evaporation water fluxes:
...
mrun_res <- mrunLWFB90(paramvar = paramvar,
                       param_b90 = param_b90,
                       cores = 5,
                       options_b90 = options_b90, # the following args are passed to runLWFB90
                       climate = slb1_meteo,
                       soil = soil,
                       output = output, 
...
legend("topleft", legend = c("approx.method = 'constant'", "approx.method = 'linear'", "approx.method = 'linear', use_growthperiod = TRUE"), 
       ...
       bty = "n")
arrows( x0 = as.Date(paste(param_b90$budburstdoy, years),format = "%j %Y"),
     y0 = c(param_b90$heightini,param_b90$height[1:2])-0.5, y1 = c(param_b90$heightini,param_b90$height[1:2])-0.1, length = 0.15, code = 2, xpd =T)
text(x = as.Date(paste(param_b90$budburstdoy, years), format = "%j %Y"),
     y = c(param_b90$heightini,param_b90$height[1:2])-0.7, paste("doy", param_b90$budburstdoy), xpd = T)
...
param_b90$rootden.table <- data.frame(
...
roots_table <- MakeRelRootDens(soilnodes = c(max(slb1_soil$upper), slb1_soil$lower), 
                              method = options_b90$root_method, 
                              rootdat = param_b90$rootden.table)
```
